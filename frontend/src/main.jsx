...
import { CarFront, CircleUserRound, Gauge, LogOut, Plus, Search, ShieldCheck, SlidersHorizontal, X, User, MapPin, Phone, Mail, Save } from 'lucide-react'
import './styles.css'
...
function Dashboard({ session, onLogout, onSessionUpdate }) {
  ...
  const purchase = async v => { 
    try { 
      ...
      setOrders(prev => [{ id: Date.now(), vehicle: v, date: new Date().toISOString(), status: 'Reserved' }, ...prev])
    } catch (e) { 
      setNotice(e.message) 
    } 
  }
        ...
        <>
          ...
          <section className="inventory-grid">
            {orders.length === 0 ? (
              ...
              orders.map(order => (
                <article key={order.id} className="vehicle-card">
                  <div className="vehicle-info">
                    <div>
                      <p className="vehicle-make">{order.vehicle.make}</p>
                      <h3>{order.vehicle.model}</h3>
                    </div>
                    <p className="price">{money.format(order.vehicle.price)}</p>
                  </div>
                  <div className="vehicle-footer">
                    <span className="stock">{order.status}</span>
                    <span style={{ fontSize: '0.85rem', color: '#888' }}>{new Date(order.date).toLocaleDateString()}</span>
                  </div>
                </article>
              ))
            )}
